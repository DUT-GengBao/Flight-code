...
static void PPM_Decode(void)
{
	...
	/*获取捕获值*/	
	PeriodVal1 = ROM_TimerValueGet( WTIMER0_BASE , TIMER_B );
...
void Drv_PpmInit(void)
{
	...
	ROM_TimerLoadSet( WTIMER0_BASE , TIMER_B , 0xffff );
	ROM_TimerPrescaleSet( WTIMER0_BASE , TIMER_B , 0xff );
	/*开启定时器中断*/
	TimerIntRegister(WTIMER0_BASE,  TIMER_B , PPM_Decode);	
	ROM_IntPrioritySet( INT_WTIMER0B , USER_INT6);
