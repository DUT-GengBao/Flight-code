...
#define LT_KP                  (0.7f)  //比例项
#define LT_KD                  (0.3f)  //微分项
...
#define LLH_TIME        500   //判断目标丢失的保持时间。
#define CONFIRM_TIMES     5    //重确认特征判断的次数，确认次数越多，越能避免误检测，但是也可能导致检测不到。
#define YAW_PAL_DPS       0.4    //航向角速度，度每秒
#define FORWARD_VEL       21    //前进速度
...
{
	//开启控制的条件，可以自己修改
	if(opmv.mode_sta==2 && opmv_ct_sta.en  && openmv_switch_flag == 1)
	{
		//寻线数据旋转解耦合
...
static void ANO_LTracking_Calcu(u8 *dT_ms,s32 relative_height_cm)
{
	...
	{
		relative_height_cm_valid = relative_height_cm-110;
	}
	else
	{
		//null
...
void ANO_LTracking_Ctrl(u8 *dT_ms,u8 en)
{
	...
	if(en)
	{
		...
		{
			//
			ano_opmv_lt_ctrl.exp_yaw_pal_dps = -opmv.lt.angle *1.6;
