...
//
#define CBT_KPX                  (0.7f)  //比例项
#define CBT_KDX                 (0.3f)  //微分项
#define CBT_KFX                  (0.5f)  //前馈项
//水平
#define CBT_KPY                 (1.0f)  //比例项
#define CBT_KDY                  (0.3f)  //微分项
#define CBT_KFY                  (0.6f)  //前馈项
//需要操作赋值的外部变量：
...
{
//	UARTprintf("%d %d %d",opmv2.mode_sta,opmv_ct_sta.en, openmv_switch_flag);
	//开启控制的条件，可以自己修改
	if(opmv2.mode_sta==1 && opmv_ct_sta.en && openmv_switch_flag == 2)
	{
		//跟踪数据旋转解耦合
		ANO_CBTracking_Decoupling(&dT_ms,IMU_ROL,IMU_PIT);
		//跟踪数据计算
		ANO_CBTracking_Calcu(&dT_ms,(s32)opmv2.cb.pos_y);
	}
	else
	{
		//reset
...
static void ANO_CBTracking_Decoupling(u8 *dT_ms,float rol_degs,float pit_degs)
{
	...
	//有识别到目标
	if(opmv2.cb.sta != 0)//(opmv.cb.color_flag!=0)
	{
		//
	...
	//换到飞控坐标系
		...
		ano_opmv_cbt_ctrl.opmv_pos[0] =  (opmv2.cb.pos_y);
		ano_opmv_cbt_ctrl.opmv_pos[1] = 	(opmv2.cb.pos_x);
...
//		ano_opmv_cbt_ctrl.rp2pixel_val[1] = LIMIT(ano_opmv_cbt_ctrl.rp2pixel_val[1],-80,80);//宽度160pixel
	...
	{
		//得到平移偏移量，并低通滤波
		decou_pos_pixel_lpf[0][0] += 0.2f *((ano_opmv_cbt_ctrl.opmv_pos[0] ) - decou_pos_pixel_lpf[0][0]);
		decou_pos_pixel_lpf[0][1] += 0.2f *((ano_opmv_cbt_ctrl.opmv_pos[1] ) - decou_pos_pixel_lpf[0][1]);
...
//		decou_pos_pixel_lpf[0][1] += 0.2f *((ano_opmv_cbt_ctrl.opmv_pos[1] - ano_opmv_cbt_ctrl.rp2pixel_val[1]) - decou_pos_pixel_lpf[0][1]);
		...
		//赋值
//		LPF_1_(0.5f,dT_s,decou_pos_pixel_lpf[1][0],ano_opmv_cbt_ctrl.decou_pos_pixel[0]);
//		LPF_1_(0.5f,dT_s,decou_pos_pixel_lpf[1][1],ano_opmv_cbt_ctrl.decou_pos_pixel[1]);
...
//		ano_opmv_cbt_ctrl.decou_pos_pixel[0] = ano_opmv_cbt_ctrl.decou_pos_pixel[1] = 0;
	...
	//
	UARTprintf("%d %d\r\n",(int)(decou_pos_pixel_lpf[1][0]*100),(int)(ano_opmv_cbt_ctrl.decou_pos_pixel[0]*100));
}
...
static void ANO_CBTracking_Calcu(u8 *dT_ms,s32 relative_height_cm)
{
	...
	{
		relative_height_cm_valid = relative_height_cm - 30;
	}
	else
	{
		//null
	...
	//得到地面偏差，单位厘米
	ano_opmv_cbt_ctrl.ground_pos_err_h_cm[0] = relative_height_cm;
...
//	
//	UARTprintf("%d %d\r\n",(int)(ano_opmv_cbt_ctrl.ground_pos_err_h_cm[0]),(int)(ano_opmv_cbt_ctrl.ground_pos_err_h_cm[1]));
}
...
float exp_velocity_h_x,exp_velocity_h_y;
...
void ANO_CBTracking_Ctrl(u8 *dT_ms,u8 en)
{
	...
	if(en)
	{
		//距离偏差PD控制和速度前馈
		exp_velocity_h_x\
		= CBT_KFX *ano_opmv_cbt_ctrl.target_gnd_velocity_cmps[0]\
		+ CBT_KPX *(ano_opmv_cbt_ctrl.ground_pos_err_h_cm[0])\
		+ CBT_KDX *ano_opmv_cbt_ctrl.ground_pos_err_d_h_cmps[0];
		LPF_1_(1,0.02,exp_velocity_h_x,ano_opmv_cbt_ctrl.exp_velocity_h_cmps[0]);
		
		exp_velocity_h_y\
		= CBT_KFY *ano_opmv_cbt_ctrl.target_gnd_velocity_cmps[1]\
		+ CBT_KPY *(ano_opmv_cbt_ctrl.ground_pos_err_h_cm[1])\
		+ CBT_KDY *ano_opmv_cbt_ctrl.ground_pos_err_d_h_cmps[1];
		
		LPF_1_(1,0.02,exp_velocity_h_y,ano_opmv_cbt_ctrl.exp_velocity_h_cmps[1]);
		
		ano_opmv_cbt_ctrl.exp_velocity_h_cmps[0] = LIMIT(ano_opmv_cbt_ctrl.exp_velocity_h_cmps[0],-40,40);
		ano_opmv_cbt_ctrl.exp_velocity_h_cmps[1] = LIMIT(ano_opmv_cbt_ctrl.exp_velocity_h_cmps[1],-40,40);
//		UARTprintf("%d \r\n",(int)(ano_opmv_cbt_ctrl.ground_pos_err_h_cm[0]*100));
	}
	else
	{
