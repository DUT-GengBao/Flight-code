...
#include "ws2812.h"
#include "uartstdio.h"
u8 openmv_ct_en = 1; //默认开启openmv控制,自己添加
...
u8 openmv_switch_flag = 0;		//切换2个openmv，1为底下的openmv，2为前面的openmv
//参数设定：
...
void ANO_OPMV_Ctrl_Task(u8 dT_ms)
{
	...
	//不同模式执行不同功能
	if(opmv2.mode_sta==1 && openmv_switch_flag == 2 )			//找点
	{
		color(1,1);
		//
	...
	else if(opmv.mode_sta == 2 && openmv_switch_flag == 1)		//巡线
	{
		color(2,2);
//		UARTprintf("LT...");
		//
	...
	else if(opmv.mode_sta==1 && openmv_switch_flag == 1 )			//找点  挂砝码定点    如果opmv1的模式为追点  
	{
		color(1,1);
		//
		opmv_ct_sta.reset_flag = 0;
